...
func NewMessageStore() (*MessageStore, error) {
	...
	// base creada antes de que hubiera direct_path se queda sin la columna y
	// cada insercion falla. Se añade aqui en vez de pedir que se borre la
	// base: dentro esta todo el historial ya sincronizado.
...
// extractMediaInfo pulls out what is needed to download an attachment later.
//
// directPath is returned alongside url and must be stored as WhatsApp gives
// it: it carries its own query string (?ccb=..&oh=..&oe=..) holding the CDN's
// authentication. whatsmeow appends its parameters with "&", so a path
// stripped of that query yields an unauthenticated URL and the CDN answers
// 403. Deriving the path from the url by cutting at "?" — which is what this
// did before — loses exactly those parameters.
...
func downloadMedia(client *whatsmeow.Client, messageStore *MessageStore, messageID, chatJID string) (bool, string, string, string, error) {
	...
	// El direct path guardado es el que da WhatsApp, con su query string de
	// autenticacion intacta. Solo si falta —mensajes anteriores a que se
	// empezara a guardar— se recurre a derivarlo de la url, sabiendo que ese
	// camino pierde los parametros oh/oe y acaba en 403.
	if directPath == "" {
